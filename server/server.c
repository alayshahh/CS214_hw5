...
void listenInputs(int connfd, int player) {
    ...
    while ((n = Rio_readlineb(&rio, &dir, sizeof(Direction))) != 0) {
        printf("Player %d moved ", player);
        switch (dir) {
            case UP:
                printf("UP\n");
                break;
...
// adds a player to the game mid level, making sure the player is not taking occupied space
// int addPlayer(TILE grid[GRIDSIZE][GRIDSIZE], Position playerPositions[MAX_CLIENTS], int player) {
//     int x = random10();
//     int y = random10();
//     playerPositions[player].x = x;
//     playerPositions[player].y = y;
//     for (int i = 0; i < MAX_CLIENTS; i++) {
//         if ((playerPositions[i].x == x && playerPositions[i].y == y) || grid[x][y] == TILE_TOMATO) {  // check if space is occupied, if it is try again
//             return FALSE;
//         }
//     }
//     return TRUE;
...
// event loop, inits the game for all connected players, and then handles all gmae logic, passing on the updared player positions after each move to each client
    ...
    while (1) {  // inifite loop ->
        ...
        while (!shouldExit) {
            ...
            for (int i = 0; i < MAX_CLIENTS; i++) {  // move all disconnected players off the board & add new players to the board & send board info
                ...
                //     int added;
                //     while ((added = addPlayer(grid, playerPositions, i)) == FALSE) {
                //         ;
                //     }
                //     // write the grid
                //     Rio_writen(sockets[i], GRID, sizeof(GRID));
            ...
            if (numPlayers == 0) {
                // if there are no more players, then we want to exit the game, so new players can join
                shouldExit = TRUE;
            }
